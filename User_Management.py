...
def verify_user_admin():
    ...
    while item > len(temp_data_list):
        item = int(
            input("No user selected! Enter number to select the user to verify:")
        )
...
def disable_user_access():
    ...
    while confirm_disable == 2:
        ...
        while item > len(temp_data_list):
            item = int(
                input("No user selected! Enter number to select the user to disable:")
            )
        if item <= len(temp_data_list):
            item = item - 1
...
def report():
    ...
    print(
        f"Verify new admin or customers account(Superuser): {no_verify_new_admin_or_customers}"
    )
...
def user_mangement_menu_superuser(current_user):
    ...
    if choice == 2:
        ...
        write_user_usage(
            current_user[1], current_user[4], "verify_new_admin_or_customers"
        )
    if choice == 3:
        modify_user_details()
        write_user_usage(
