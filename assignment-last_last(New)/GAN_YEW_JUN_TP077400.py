...
def verify_user_admin(func_back_to,current_user):
    ...
    print(f"{'No':<5}{'ID number':<14}{'Username':<14}{'Status':<10}{'User type':<10}{'Name':<20}{'Phone number':<15}{'Address'}")
    print("="*100)
    for length in range(len(temp_data_list)):
        user_data = temp_data_list[length]
        row = [str(length + 1)] + user_data[:7]
        print(f"{row[0]:<5}{row[1]:<14}{row[2]:<14}{row[3]:<10}{row[4]:<10}{row[5]:<20}{row[6]:<15}{row[7]}")
...
def modify_user_details(func_back_to,current_user):
    ...
    user_data_list=conv_to_list(file_data)
    # print users data
    print(f"{'No':<5}{'ID number':<15}{'Username':<15}{'password':<10}{'Status':<10}{'User type':<10}{'Name':<20}{'Phone number':<15}{'Address'}")
    print("="*110)
    for length in range(len(user_data_list)):
        user_data = user_data_list[length]
        data = [str(length + 1)] + user_data
        print(f"{data[0]:<5}{data[1]:<15}{data[2]:<15}{data[3]:<10}{data[4]:<10}{data[5]:<10}{data[6]:<20}{data[7]:<15}{data[8]}\n")
    # select user to modify 
    item=input("Enter number to select the user to modify:")
    while True:
        if item.isdigit() is False:
            item=input("Please enter number to select user to modify:")
        elif int(item) not in range(1,len(user_data_list)+1):
...
def disable_user_access(func_back_to,current_user):
    ...
    while confirm_disable==2:
        ...
        confirm_disable=input("Enter 1 to disable(Enter b to retun menu): ")
        while confirm_disable not in ["1","b"]:
            confirm_disable=input("Invalid input!! Enter 1 to disable(Enter b to retun menu): ")
