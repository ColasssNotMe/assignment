...
def check_username(new_username, func_back_to, current_user):
    ...
    while True:
        ...
        if username_exists:
            ...
            new_username = input(
                "--Please enter another username(Enter b to retun menu): "
            )
        else:
            break
    return new_username
...
def user_types(func_back_to, current_user):
    ...
    while choice not in ["1", "2", "3", "4", "b"]:
        choice = input(
            "Invalid user type! Please enter 1, 2, 3 or 4(Enter b to retun menu): "
        )
    if choice == "1":
        return "customer"
    if choice == "2":
...
def check_name(new_users_name, func_back_to, current_user):
    while True:
        if not new_users_name:
            ...
            new_users_name = input(
                "--Please enter a user's name(Enter b to retun menu): "
            )
    ...
    while True:
        if new_phone_number == "":
            ...
            new_phone_number = input(
                "--Please enter a phone number(Enter b to retun menu): "
            )
            ...
            new_phone_number = input(
                "--Please enter another phone number(Enter b to retun menu): "
            )
...
def check_address(new_user_address, func_back_to, current_user):
    while True:
        if new_user_address == "":
            ...
            new_user_address = input(
                "--Please re-enter Address(Enter b to retun menu): "
            )
...
def add_user(func_back_to, current_user):
    ...
    new_users_phone_no = input(
        "Please enter user's phone number(Enter b to retun menu): "
    )
    ...
    for length in range(len(user_data_list)):
        ...
        if status == "pending" and user_type == "customer" or user_type == "staff":
            temp_data_list.append(
                user_data_list[length][:2] + user_data_list[length][3:8] + [length]
            )
...
def select_and_edit(func_back_to, current_user):
    ...
    if choice in ["1", "2", "3", "4", "5", "6", "7", "8"]:
        ...
        if choice == 0:
            ...
            updatedata = input(
                "Please input a default password(Enter b to retun menu): "
            )
    ...
    while confirm_disable == 2:
        ...
        while True:
            if item.isdigit() is False:
                ...
                item = input(
                    "No user selected! Enter number to select the user to disable: "
                )
            else:
                break
        item = int(item)
        ...
        confirm_disable = input(
            "Enter 1 to disable, enter 2 to re-select(Enter b to retun menu): "
        )
        while confirm_disable not in ["1", "2", "b"]:
            confirm_disable = input(
                "Invalid input!! Enter 1 to disable, enter 2 to re-select(Enter b to retun menu): "
            )
...
def show_user_usage():
    with open("user_usage.txt", "r") as user_usage:
        ...
        for record in log_list:
            ...
            print(
                f"{recordList[0]}: {recordList[1]}({recordList[2]})-----{recordList[3]}"
            )
...
def inventory_order_status():
    ...
    # Print headers
    print(
        f"{'Order ID':<10}{'Item Name':<20}{'Quantity':<10}{'Price(RM)':<12}{'Status':<12}"
    )
    for item in purchase_orders:
        print(
            f"{item[0].center(9):<10}{item[1]:<20}{str(item[2]).center(8):<10}{str(item[3]).center(9):<12}{item[4]:<12}"
        )
...
def user_mangement_menu_superuser(current_user):
    ...
    if choice == "2":
        if waiting == 0:
            ...
            verify_user_superuser(
                func_back_to=user_mangement_menu_superuser, current_user=current_user
            )
            write_user_usage(
                current_user[1], current_user[4], "verify_new_user(superuser)"
            )
            back_to_menu(user_mangement_menu_superuser, current_user=current_user)
    if choice == "3":
        modify_user_details(
        ...
        write_user_usage(
            current_user[1], current_user[4], "modify_user_personal_details"
        )
        back_to_menu(user_mangement_menu_superuser, current_user=current_user)
    if choice == "4":
        disable_user_access(
    ...
    if choice == "5":
        write_user_usage(
            current_user[1], current_user[4], "inquiry_of_user's_system_usage"
        )
        show_user_usage()
        back_to_menu(user_mangement_menu_superuser, current_user=current_user)
    if choice == "6":
    ...
    if choice == "7":
        ...
        write_user_usage(
            current_user[1],
            current_user[4],
            "stock_purchase_order_status(admin/superuser)",
        )
        back_to_menu(user_mangement_menu_superuser, current_user=current_user)
    if choice == "8":
...
def user_mangement_menu_admin(current_user):
    ...
    if choice == "1":
        if waiting == 0:
            ...
            verify_user_admin(
                func_back_to=user_mangement_menu_admin, current_user=current_user
            )
